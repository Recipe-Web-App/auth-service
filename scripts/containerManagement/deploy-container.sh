...
CONFIG_DIR="k8s"
SECRET_NAME="auth-service-secrets" # pragma: allowlist secret
...
# Set configuration variables to override Go service defaults
...
# Redis Configuration
export REDIS_URL="${REDIS_URL:-redis://redis.default.svc.cluster.local:6379}"
...
print_separator "="
echo -e "${CYAN}🟢 Building Go Docker image: ${FULL_IMAGE_NAME} (inside Minikube Docker daemon)${NC}"
print_separator '-'
...
print_separator "="
echo -e "${CYAN}📦 Deploying Auth Service container...${NC}"
print_separator "-"

kubectl apply -f "${CONFIG_DIR}/deployment.yaml"

print_separator "="
echo -e "${CYAN}🌐 Exposing Auth Service via ClusterIP Service...${NC}"
print_separator "-"
...
print_separator "-"
print_status "ok" "Auth Service is up and running in namespace '$NAMESPACE'."

print_separator "="
echo -e "${CYAN}🔗 Setting up /etc/hosts for auth-service.local...${NC}"
print_separator "-"

MINIKUBE_IP=$(minikube ip)
if grep -q "auth-service.local" /etc/hosts; then
  echo -e "${YELLOW}🔄 Updating /etc/hosts for auth-service.local...${NC}"
  sed -i "/auth-service.local/d" /etc/hosts
else
  echo -e "${YELLOW}➕ Adding auth-service.local to /etc/hosts...${NC}"
fi
echo "$MINIKUBE_IP auth-service.local" | tee -a /etc/hosts
